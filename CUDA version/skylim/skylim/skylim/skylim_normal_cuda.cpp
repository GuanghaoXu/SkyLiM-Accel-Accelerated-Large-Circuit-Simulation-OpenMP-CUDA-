...
__global__ void branch_kernel(branch* d_ckbrch, node* d_cknod, int Nb, double dt, int imethod) {
	...
	if (i <= Nb) {
		...
		// 使用 atomicAdd(ptr, val) 进行线程安全的更新
...
int main(int argc, char* argv[])
{
	...
	// simulator monitors probe nodes (ps1, ps2, ps3, ps4) Local cache
	// 1-based indexing,ignore index 0
	node h_monitored_nodes[5]; 
	...
	while (true) {
		...
		while (tim <= dur) {
			...
			if (version == 1){
				...
				branch_kernel <<< blocksPerGrid, threadsPerBlock >>> (d_ckbrch, d_cknod, Nb, dt, imethod);
			...
			if (version == 1) {
				...
				node_kernel << <blocksPerGrid, threadsPerBlock >> > (d_cknod, Nn, tim, dt, imethod, idvs, magi, del, ris, fal, wid);
				checkCudaErrors(cudaPeekAtLastError());
				checkCudaErrors(cudaDeviceSynchronize());

				// Memcpy monitored nodes back to Host
				checkCudaErrors(cudaMemcpy(&h_monitored_nodes[1], &d_cknod[ps1], sizeof(node), cudaMemcpyDeviceToHost));
				checkCudaErrors(cudaMemcpy(&h_monitored_nodes[2], &d_cknod[ps2], sizeof(node), cudaMemcpyDeviceToHost));
				checkCudaErrors(cudaMemcpy(&h_monitored_nodes[3], &d_cknod[ps3], sizeof(node), cudaMemcpyDeviceToHost));
				checkCudaErrors(cudaMemcpy(&h_monitored_nodes[4], &d_cknod[ps4], sizeof(node), cudaMemcpyDeviceToHost));
			} 
